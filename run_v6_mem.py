"""
DisorderNet v6: Memory-optimized with ESM-2 + Physics.
Uses 1500 proteins, PCA-48, 5 ESM window scales + variance.
"""
...
def main():
    print("="*70)
    print("DisorderNet v6: ESM-2 + Physics (Memory-Optimized)")
    print("="*70)
    ...
    tr=sum(p["length"] for p in proteins); td=sum(sum(p["disorder_labels"]) for p in proteins)
    print(f"Proteins: {len(proteins)} | Residues: {tr:,} ({100*td/tr:.1f}% dis)")
    
    # PCA
    print(f"PCA -> {ESM_PCA} dims...")
    pca=IncrementalPCA(n_components=ESM_PCA,batch_size=10000)
    si=rng.choice(len(proteins),min(800,len(proteins)),replace=False)
    ch=[np.load(os.path.join(EMB_DIR,f"{proteins[i]['disprot_id']}.npy")).astype(np.float32)[:proteins[i]["length"]] for i in si]
    pca.fit(np.vstack(ch)); vexp=pca.explained_variance_ratio_.sum()
    print(f"Var: {vexp:.3f}"); del ch; gc.collect()
    
    # Features
    print("Features...")
    t0=time.time(); pf=[]; pl=[]
    for i,p in enumerate(proteins):
        if (i+1)%300==0: print(f"  {i+1}/{len(proteins)}")
        L=p["length"]; seq=p["sequence"][:L]
        ph=phys(seq)
        emb=np.load(os.path.join(EMB_DIR,f"{p['disprot_id']}.npy")).astype(np.float32)[:L]
        ep=pca.transform(emb)
        # ESM at 4 scales + 2 variance scales
        c=np.concatenate([ph,ep,wavg(ep,4),wavg(ep,12),wavg(ep,25),wvar(ep,8),wvar(ep,20)],1)
        pf.append(c.astype(np.float32))
        pl.append(np.array(p["disorder_labels"][:L],dtype=np.float32))
    ndim=pf[0].shape[1]; print(f"Done {time.time()-t0:.0f}s. Dim: {ndim}")
    ...
    for fold,(tr_i,va_i) in enumerate(gkf.split(range(n),range(n),range(n))):
        print(f"\nFold {fold+1}/5")
        X_val=np.nan_to_num(np.vstack([pf[i] for i in va_i]),0).astype(np.float32)
        y_val=np.concatenate([pl[i] for i in va_i])
        
        X_tr_f=np.nan_to_num(np.vstack([pf[i] for i in tr_i]),0).astype(np.float32)
        y_tr_f=np.concatenate([pl[i] for i in tr_i])
        ...
        X_tr=X_tr_f[keep]; y_tr=y_tr_f[keep]; del X_tr_f,y_tr_f; gc.collect()
        
        spw=(len(y_tr)-y_tr.sum())/max(y_tr.sum(),1)
        
        dt=lgb.Dataset(X_tr,label=y_tr,free_raw_data=True)
        dvl=lgb.Dataset(X_val,label=y_val,reference=dt,free_raw_data=True)
        lm=lgb.train({'objective':'binary','metric':'auc','num_leaves':127,'max_depth':8,
            ...
            'verbose':-1,'n_jobs':2,'seed':SEED},
            dt,700,valid_sets=[dvl],callbacks=[lgb.early_stopping(25,verbose=False),lgb.log_evaluation(0)])
        lp=lm.predict(X_val); del dt,dvl; gc.collect()
        ...
        if fold==0:
            ...
            t15=np.argsort(imp)[-15:][::-1]
            print(f"  ESM in top 15: {sum(1 for t in t15 if t>=118)}/15")
        del X_val,y_val,lm,xm; gc.collect()
    
    ya=np.concatenate(ayt); pa=np.concatenate(ayp)
    pm=evaluate(ya,pa); avs=[m["auc_roc"] for m in fm]; aps=[m["avg_precision"] for m in fm]
    our=pm["auc_roc"]
    
    print(f"\n{'='*70}\nFINAL RESULTS\n{'='*70}")
    print(f"CV: AUC={np.mean(avs):.4f}±{np.std(avs):.4f}")
    for k in ["auc_roc","avg_precision","f1","mcc","balanced_acc"]:
        vs=[m[k] for m in fm]; print(f"  {k:20s}: {np.mean(vs):.4f}±{np.std(vs):.4f}")
    print(f"\nPooled ({len(ya):,} res):")
    for k,v in pm.items(): print(f"  {k:20s}: {v:.4f}")
    
    bm=[("AF3-pLDDT (CAID3, rank 13)",0.747),("AF2-pLDDT (CAID3, rank 11)",0.770),
        ("IUPred3",0.789),("DisorderNet v4 (no ESM)",0.794),
        ("flDPnn (top CAID1/2)",0.814),("DisorderNet v5 (ESM 32d)",0.823),
        ("SETH (ProtT5+CNN)",0.830),("flDPnn3a (CAID3)",0.871),
        ("ESM2_35M-LoRA (CAID1)",0.868),("ESM2_650M-LoRA",0.880),
        ("ESMDisPred (CAID3 SOTA)",0.895),("DisorderNet v6 (OURS)",our)]
    
    print(f"\n  {'Method':<35s} {'AUC':>7s} {'vs AF3':>8s}")
    print("  "+"-"*53)
    for n,a in bm:
        m="<<<" if "OURS" in n else ""
        print(f"  {n:<33s} {a:>7.3f} {((a-0.747)/0.747)*100:>+7.1f}% {m}")
    
    print(f"\n  All>AF3:{all(v>0.747 for v in avs)} All>AF2:{all(v>0.770 for v in avs)} All>flDPnn:{all(v>0.814 for v in avs)}")
    print(f"  Folds: {[f'{v:.4f}' for v in avs]}")
    
    results={"model":"DisorderNet_v6","pooled":{k:float(v) for k,v in pm.items()},
             "fold_aucs":[float(v) for v in avs],"fold_aps":[float(v) for v in aps],
             "cv_mean_auc":float(np.mean(avs)),"cv_std_auc":float(np.std(avs)),
             "n_proteins":len(proteins),"n_features":ndim,"esm_pca":ESM_PCA,
    ...
    np.save(os.path.join(RESULTS_DIR,"y_true.npy"),ya); np.save(os.path.join(RESULTS_DIR,"y_pred.npy"),pa)
