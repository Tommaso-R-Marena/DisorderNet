...
Label error rates have been measured for the benchmarks the field actually uses.
...
error rates from 0.15\% (MNIST) to 10.12\% (QuickDraw). Their rates are lower
bounds, and substituting them gives capacities of \textbf{nine} for ImageNet,
nine for CIFAR-100, five for QuickDraw and thirteen for Amazon Reviews
(Fig.~\ref{fig:capacity}d). Northcutt \emph{et al.} showed empirically that
correcting the labels reorders these leaderboards; the bound explains why that
is not a curable defect of any particular analysis.

Whether a benchmark can do better depends on the statistic it chose, and this is
the paper's second result. For an average of per-item losses --- top-1 accuracy
--- there is no escape: an adversarial placement of the same error budget moves
the score by the full $\eps$. But a \emph{ranking} statistic over pairs within
a group is corrupted only when \emph{both} members of a pair are mis-annotated,
in opposite directions. That is a structural constraint, not a distributional
assumption, and it makes the noise exactly second-order.
...
The decomposition is exact, and its first term has a property that makes it more
than bookkeeping. \code{auc\_within\_strictMono\_invariant} (Lean~4,
\code{sorry}-free): $\mathrm{AUC}_{\text{within}}$ is invariant under any
per-protein strictly monotone recalibration, and
\code{auc\_pooled\_shift\_diff} shows the entire effect of such a recalibration
falls in the other term. The split is therefore a partition of CAID's statistic
...
Nothing in \code{card\_le\_benchCapacity} is about proteins. Top-1 accuracy on
a test set is an average over $n$ items on the grid $1/n$: flipping one label
moves it by $1/n$, so a wrong fraction $\eps$ moves it by at most $\eps$, and
two models must differ by more than $2\eps$ before the observation certifies
their order. The theorem applies verbatim.

Substituting the validated error rates of Northcutt \emph{et
al.}\cite{northcutt} (Fig.~\ref{fig:capacity}d; Supplementary Table~S17):
...
Two things this does not say. It does not say any published ranking is wrong: it
says the data cannot establish it, and if label errors happened to shift every
model equally the ordering would survive. That is an assumption about the errors,
and Northcutt \emph{et al.} measured the opposite --- correcting the labels
reorders these leaderboards, and systematically, with higher-capacity models
faring worse once corrected. Nor does it say benchmarking is futile: it says a
benchmark should publish its capacity alongside its ranking, and stop printing
places it cannot support.

Sampling error in $\eps$ propagates to the capacity, and we carry it rather than
dropping it. Bootstrapping our own estimate over the 159 proteins gives
$\eps = 0.0649$, 95\% CI $[0.0497, 0.0814]$, and a capacity of 8, 95\% CI
$[7, 11]$; the pairwise rate gives 52, 95\% CI $[37, 119]$. Even at the
pessimistic end of both intervals the pairwise protocol resolves five times more
of the field than the residue-level one.
...
runs from 32.0\% of the protein flagged to 97.3\%. A second rule buys the
identical guarantee by flagging a fixed \emph{quantile of each protein's own
scores}, which depends only on the ordering inside a chain and is therefore
calibration-invariant. The two costs price the two abilities the decomposition
separates, and their difference is what protein-level calibration is worth
operationally (Fig.~\ref{fig:operating}b). The leaders lean on it twice as hard
as we do, which yields the sharpest comparison in the paper
...
The same structure appears one step downstream, when a predictor is run over a
proteome and asked which regions are disordered. A screen's candidates share a
calibration run, a model and a training set, so under arbitrary dependence
Benjamini--Hochberg\cite{bh} needs the harmonic correction\cite{by}. We prove
that from scratch in a finite setting, assuming only that a true null's p-value
is valid (\code{selfConsistent\_fdr\_le\_harmonic}), and prove it cannot be
...
two candidates on (Fig.~\ref{fig:screen}a,c).
...
$\alpha$ from a region-level procedure at $\alpha/H_M$, with the saving bounded
below by $\log b - 1$ (\code{harmonic\_gain\_log}; Fig.~\ref{fig:screen}b).
Running it --- conformal p-values, 50 splits, CAID3 Disorder-PDB --- the
region-level screen reports 2{,}942 residues at a realised false discovery
proportion of 0.024 and power 0.568, against 203 residues and power 0.036 for
the residue-level screen at the same guaranteed rate: \textbf{14.5$\times$ the
discoveries}. A third constraint dominates both and is absent from the theory: a
conformal p-value cannot fall below $1/(n_{\text{cal}}{+}1)$, so no discovery is
possible unless $n_{\text{cal}} + 1 \ge m H_m/\alpha$ --- \emph{linear} in the
number of hypotheses where the correction is only logarithmic. Details,
including the full comparison against Bonferroni and uncorrected BH, are in
...
the full family of thirty comparisons (Fig.~\ref{fig:model}a): Linker
$+0.1364$ ($p = 3.1\times10^{-5}$, 29 of 31 targets) through Binding-IDR
$+0.2253$ ($p = 0.050$).
...
methods on annotations of finite accuracy has a capacity, that capacity is
$\lceil 1/2\eps \rceil$ regardless of how many items it scores, and past it the
ordering is not a function of the observations. On the ten benchmarks whose
label-error rates have been measured, every one is over its capacity and most
are over it by two orders of magnitude. The converse is what separates this from a power calculation:
...
The screening result is the same principle one step downstream: a deflation
factor set by how many hypotheses are stated, attained and so not arguable away,
and a report whose false discovery proportion does not change with the unit it
is read at. Two independent instances of one claim --- \textbf{where labels are
noisy or tests are many, what a study can resolve depends on a unit that is
usually chosen by convention rather than by argument.}

Our recommendation to any benchmark is to publish its capacity beside its
ranking and stop printing places it cannot support; and, where its statistic
admits the pairing structure, to score pairs. For CAID specifically, report the
within-protein component alongside the pooled AUC. CAID4's references are
unpublished, so both are adoptable now.
