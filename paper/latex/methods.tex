...
\code{propext}, \code{Classical.choice} and \code{Quot.sound}. The proofs were
constructed with Aristotle (Harmonic) from statements specified by the author;
being kernel-checked on the standard axioms, their correctness does not depend
on how they were produced, while the choice of what to formalise does and is
the author's. The development covers:
...
A full inventory is Supplementary Note~S6. Five further files close the
statements this work used before they were proved: \code{DiscordantImbalance}
(the closed form without a balance hypothesis), \code{ConformalPValueBH} (the
composition of split conformal with the dependent screen), \code{SeparationVsChain}
(what the capacity bounds, and what it does not), \code{CapacityConfidence}
(capacity from an estimated rate) and \code{UnresolvableCount} (how many
comparisons are undecidable). \textbf{Nothing in this paper now rests on a
...
computes the statistic the theorems describe. \code{weighted\_ne\_unweighted\_instance}
plays the same role for the protocol's statistic: a five-residue instance where
the pair-weighted and unweighted means differ (1/2 against 1/3), so the two
invariance theorems are separately necessary rather than one implying the other.
