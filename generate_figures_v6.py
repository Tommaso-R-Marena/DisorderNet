...
Reads ``y_true.npy`` / ``y_pred.npy`` / ``metrics.json`` (produced by
``run_v6_mem.py``) from the results dir and writes ROC/PR, benchmark, progression,
and fold-stability figures. Importable (no work at import time) and testable.
"""
...
def generate(R):
    """Generate all v6 figures into results dir ``R``. Returns list of paths."""
    R = str(R)
    yt = np.load(os.path.join(R, "y_true.npy"))
    yp = np.load(os.path.join(R, "y_pred.npy"))
    with open(os.path.join(R, "metrics.json")) as f:
        M = json.load(f)
    ...
    p1 = os.path.join(R, 'fig1_roc_pr.png')
    plt.tight_layout(); plt.savefig(p1, dpi=200, bbox_inches='tight'); plt.close(); saved.append(p1)
    ...
    p2 = os.path.join(R, 'fig2_comparison.png')
    plt.tight_layout(); plt.savefig(p2, dpi=200, bbox_inches='tight'); plt.close(); saved.append(p2)
    ...
    ax.bar(xs, va, color=vc, width=0.55, edgecolor='white', lw=2)
    for i, v in enumerate(va):
        ax.text(i, v + 0.003, f'{v:.3f}', ha='center', fontsize=13, fontweight='bold')
    ...
    p3 = os.path.join(R, 'fig3_progression.png')
    plt.tight_layout(); plt.savefig(p3, dpi=200, bbox_inches='tight'); plt.close(); saved.append(p3)

    # Fig 4: Fold stability
    fig, ax = plt.subplots(figsize=(8, 5))
    faucs = M["fold_aucs"]
    ax.bar(range(1, len(faucs) + 1), faucs, color='#2563EB', width=0.6, edgecolor='white', lw=1.5, alpha=0.85)
    ...
    for i, v in enumerate(faucs):
        ax.text(i + 1, v + 0.003, f'{v:.3f}', ha='center', fontsize=11, fontweight='bold')
    ...
    ax.legend(fontsize=10); ax.set_ylim(0.72, 0.88)
    ax.set_xticks(range(1, len(faucs) + 1)); ax.grid(True, axis='y', alpha=0.2)
    p4 = os.path.join(R, 'fig4_folds.png')
    plt.tight_layout(); plt.savefig(p4, dpi=200, bbox_inches='tight'); plt.close(); saved.append(p4)
