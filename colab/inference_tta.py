...
def mc_dropout_forward_logits(
    ...
    forward_fn: callable(model, tokens, aa_idx, mask, rich_feats) -> logits
    """
    if n_passes <= 1:
        return forward_fn(model, tokens, aa_idx, mask, rich_feats)

    was_training = model.training
    model.train()
    accum = None
    for _ in range(n_passes):
        logits = forward_fn(model, tokens, aa_idx, mask, rich_feats)
...
def mc_dropout_predict_probs(
    ...
    logits = mc_dropout_forward_logits(
        model, tokens, aa_idx, mask, rich_feats, n_passes, forward_fn,
    )
